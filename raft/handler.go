...
type ProtocolManager struct {
	...
	// Storage
	iichainRaftDb *leveldb.DB             // Persistent storage for last-applied raft index
...
func NewProtocolManager(raftId uint16, raftPort uint16, blockchain *core.BlockChain, mux *event.TypeMux, bootstrapNodes []*discover.Node, joinExisting bool, datadir string, minter *minter, downloader *downloader.Downloader) (*ProtocolManager, error) {
	...
	iichainRaftDbLoc := fmt.Sprintf("%s/iichain-raft-state", datadir)
	...
	if db, err := openIichainRaftDb(iichainRaftDbLoc); err != nil {
		return nil, err
	} else {
		manager.iichainRaftDb = db
	}
...
func (pm *ProtocolManager) startRaft() {
	...
	raftConfig := &etcdRaft.Config{
		...
		// NOTE, from cockroach:
		// "PreVote and CheckIichain are two ways of achieving the same thing.

...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	isIichain := pool.chainconfig.IsIichain

	if isIichain && tx.GasPrice().Cmp(common.Big0) != 0 {
	...
	if !isIichain && !local && pool.gasPrice.Cmp(tx.GasPrice()) > 0 {
	...
	if !isIichain && tx.Gas().Cmp(intrGas) < 0 {
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if uint64(len(pool.all)) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		if !pool.chainconfig.IsIichain && pool.priced.Underpriced(tx, pool.locals) {
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	if isIichain && pool.pendingState == nil {
		pool.reset(nil, nil)
	}

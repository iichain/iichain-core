...
func (cg *callHelper) MakeCall(private bool, key *ecdsa.PrivateKey, to common.Address, input []byte) error {
	...
	signer := types.MakeSigner(params.IichainTestChainConfig, cg.header.Number)
	...
	bc, _ := NewBlockChain(cg.db, params.IichainTestChainConfig, ethash.NewFaker(), vm.Config{})
	context := NewEVMContext(msg, &cg.header, bc, &from)
	vmenv := vm.NewEVM(context, publicState, privateState, params.IichainTestChainConfig, vm.Config{})

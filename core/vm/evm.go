...
// note: Iichain, States, and Value Transfer
//
// In Iichain there is a tricky issue in one specific case when there is call from private state to public state:
...
//
// The solution is to skip this transfer of 0 value under Iichain
...
type EVM struct {
	...
	// be simplified). This is set by Iichain when it's inside a Private State -> Public State read.
	iichainReadOnly bool
	readOnlyDepth  uint
}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if evm.ChainConfig().IsIichain {
		// skip transfer if value /= 0 (see note: Iichain, States, and Value Transfer)
		if value.Sign() != 0 {
			if evm.iichainReadOnly {
...
func (env *EVM) Pop() {
	env.currentStateDepth--
	if env.iichainReadOnly && env.currentStateDepth == env.readOnlyDepth {
		env.iichainReadOnly = false
	}

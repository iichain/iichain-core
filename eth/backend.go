...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	eth.miner.SetExtra(makeExtraData(config.ExtraData, eth.chainConfig.IsIichain))
...
func makeExtraData(extra []byte, isIichain bool) []byte {
	if len(extra) == 0 {
	...
	if uint64(len(extra)) > params.GetMaximumExtraDataSize(isIichain) {
		log.Warn("Miner extra data exceed limit", "extra", hexutil.Bytes(extra), "limit", params.GetMaximumExtraDataSize(isIichain))
		extra = nil
	}
	return extra
}
